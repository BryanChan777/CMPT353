...
def main():
    data = pd.read_csv('owl_playerstats.csv').drop(columns=['name', 'role', 'teamId'])
    
    X_1 = data.drop(columns=['team']).loc[:,'playerId':'time_played_total'].values
    y_1 = data.team.values
